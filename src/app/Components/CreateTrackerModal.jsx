...
export default function CreateTrackerModal({ applications, onClose, onCreated }) {
  const [step, setStep] = useState(1); // 1 = basics, 2 = topics
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  // Step 1 fields
  const [companyMode, setCompanyMode] = useState("linked"); // "linked" | "manual"
  ...
  const [customTopics, setCustomTopics] = useState([]); // { name, category }
  ...
  const handleSubmit = async () => {
    ...
    if (dateMode === "date" && !roundDate) {
      setError("Please enter the round date.");
      return;
    }
    if (dateMode === "days" && (!daysManual || Number(daysManual) < 1)) {
    ...
    const body = {
      ...
      customTopics,
      notes,
      // Pass selected categories so the API can filter
            ...
            <>
              ...
              <div className="form-group">
                <div className="toggle-group" style={{ marginBottom: 10 }}>
                  <button
                    className={`toggle-btn ${companyMode === "linked" ? "active" : ""}`}
                    onClick={() => setCompanyMode("linked")}
                  >
                    Link to application
                  </button>
                  <button
                    className={`toggle-btn ${companyMode === "manual" ? "active" : ""}`}
              ...
              <div className="form-group">
                <div className="toggle-group" style={{ flexWrap: "wrap" }}>
                  {ROUND_NAMES.map((r) => (
                    <button
                      key={r}
                      className={`toggle-btn ${roundName === r ? "active" : ""}`}
                  ...
                  <button
                    className={`toggle-btn ${roundName === "__custom__" ? "active" : ""}`}
                    onClick={() => setRoundName("__custom__")}
                  >
                    Custom
                  </button>
                </div>
              ...
              <div className="form-group">
                <div className="toggle-group" style={{ marginBottom: 10 }}>
                  <button
                    className={`toggle-btn ${dateMode === "date" ? "active" : ""}`}
                    onClick={() => setDateMode("date")}
                  >
                    Pick exam date
                  </button>
                  <button
                    className={`toggle-btn ${dateMode === "days" ? "active" : ""}`}
                ...
                {dateMode === "date" ? (
                  ...
                  <input
                    ...
                    min={1}
                    value={daysManual}
                    onChange={(e) => setDaysManual(e.target.value)}
                  />
                )}
              </div>
            ...
            <>
              ...
              <div className="form-group">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                    Auto-add common aptitude topics
                  </span>
                  <button
                    className={`toggle-btn ${useDefault ? "active" : ""}`}
                  ...
                  <>
                    ...
                    <div className="toggle-group">
                      {["Quantitative", "Logical", "Verbal"].map((cat) => (
                        <button
                          key={cat}
                          className={`toggle-btn ${selectedCategories.includes(cat) ? "active" : ""}`}
              ...
              <div className="form-group">
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  ...
                  <button className="btn-primary" onClick={addCustomTopic} style={{ whiteSpace: "nowrap" }}>
                    Add
                  </button>
                </div>
              ...
              <div style={{
                ...
                <strong style={{ color: "var(--text-primary)" }}>
                  {dateMode === "days"
                    ? `${daysManual || "?"} days`
                    : roundDate
                    ? `${Math.max(1, Math.ceil((new Date(roundDate) - new Date()) / 86400000))} days`
                    : "? days"}
                </strong>
              </div>
            </>
